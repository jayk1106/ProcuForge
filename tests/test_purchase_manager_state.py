...
from procu_forge_buyer.subagents.purchase_manager.tools import _get_vendor_config
...
def test_get_vendor_config_with_plain_dict():
    result = _get_vendor_config(_sample_state_value())
    assert result == (_VENDOR_ID, _sample_state_value()[NEGOTIATION_CONFIG_KEY][_VENDOR_ID])


def test_get_vendor_config_with_adk_state():
    adk_state = State(_sample_state_value(), {})
    result = _get_vendor_config(adk_state)
